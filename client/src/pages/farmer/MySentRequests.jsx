...
const MySentRequests = () => {
    ...
    const fetchExchanges = async () => {
        try {
            const res = await exchangeAPI.getSent();
            setExchanges(res.data.exchanges);
        } catch (err) {
            console.error('Failed to fetch sent exchanges:', err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleComplete = async (exchangeID) => {
        ...
        try {
            ...
            // Show barter receipt if exchange is now completed
    ...
    // Show receipt for any completed exchange
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-green-50 pb-24">
            <header className="bg-gradient-to-r from-green-600 to-emerald-600 text-white py-6 px-6 shadow-lg">
                <div className="container mx-auto">
                    <BackButton className="mb-4" />
                    <div className="flex items-center justify-between">
                        <div>
                        <h1 className="text-2xl font-bold">📤 My Sent Exchanges</h1>
                        <p className="text-green-100 text-sm">{exchanges.length} exchange(s)</p>
                    </div>
                    <div className="flex gap-2">
                        ...
                        <Link to="/farmer/exchanges/received" className="bg-white/20 text-white px-3 py-2 rounded-lg text-sm font-semibold hover:bg-white/30">📥 Received</Link>
                        <Link to="/farmer/exchange/new" className="bg-white text-green-700 px-3 py-2 rounded-lg font-semibold hover:bg-green-50">+ New</Link>
                    </div>
                </div>
            </div>
        </header>
            ...
            <div className="container mx-auto px-4 py-6 max-w-xl">
                {exchanges.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <p className="text-gray-500 text-lg">No exchanges sent yet</p>
                        <Link to="/farmer/exchange/new" className="inline-block mt-4 bg-green-600 text-white px-6 py-3 rounded-lg font-semibold">Create Exchange</Link>
                    </div>
                ) : (
                            ...
                            return (
                                <div key={ex.exchangeID} className="bg-white rounded-xl shadow-md border border-gray-200 p-5">
                                    ...
                                    <div className="bg-gray-50 rounded-lg p-3 mb-3 text-sm">
                                        <p className="font-bold text-gray-700 mb-1">📞 Receiver:</p>
                                        <p>{ex.receiverName || 'N/A'} (<span className="font-mono">{ex.receiverCustomID}</span>)</p>
                                        <p>Phone: <a href={`tel:${ex.receiverPhone}`} className="text-blue-600 font-semibold">{ex.receiverPhone || 'N/A'}</a></p>
                                    </div>

                                    <div className="grid grid-cols-2 gap-3 text-sm">
                                        <div className="bg-blue-50 rounded-lg p-3">
                                            <p className="text-gray-500 text-xs mb-1">You offer</p>
                                        ...
                                        <div className="bg-orange-50 rounded-lg p-3">
                                            <p className="text-gray-500 text-xs mb-1">You want</p>
                                    ...
                                    <div className="mt-3 flex gap-2">
                                        {ex.status === 'pending' && (
                                            <button onClick={() => handleCancel(ex.exchangeID)} disabled={actionLoading === ex.exchangeID}
                                                className="flex-1 bg-red-500 hover:bg-red-600 text-white py-2 rounded-lg text-sm font-semibold disabled:opacity-50">
                                                {actionLoading === ex.exchangeID ? '...' : '🚫 Cancel'}
                                            </button>
                                        )}
                                        {ex.status === 'accepted' && !ex.requesterConfirmed && (
                                        ...
                                        {ex.status === 'accepted' && ex.requesterConfirmed && !ex.receiverConfirmed && (
